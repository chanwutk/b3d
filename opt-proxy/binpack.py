...
mask = mask.find('.//image[@name="jnc00.jpg"]')
...
bitmask = torch.from_numpy(bitmask).to('cuda:0').to(torch.bool)
...
#     frame = torch.from_numpy(frame).to('cuda:0') * bitmask
...
writer = cv2.VideoWriter('output.mp4', fourcc, 30, SIZE[::-1])

resize = Resize(size=SIZE).to('cuda:0')
...
for idx in range(LIMIT):
    ...
    # prediction = predictor.model([{'image': torch.from_numpy(frame.astype("float32").transpose(2, 0, 1)).to('cuda:0'), 'height': 750, 'width': 1333}])[0]
    ...
    marked_frame2 = np.copy(frame)
    marked_frame3 = np.copy(frame)

    grouped_bitmap, grouped_chunks = group_chunks(bitmap)

    cv2.imwrite(f'example-frames/{_idx:03d}.jpg', frame[5*CHUNK_SIZE:14*CHUNK_SIZE, 10*CHUNK_SIZE:23*CHUNK_SIZE])

    # highlight relevent chunks with neighbor chunks
    for i, j, chunk in chunks:
        if bitmap[i, j] == 1:
            color = colors[1]
            marked_frame2[bitmap_index(i):bitmap_index(i + 1), bitmap_index(j):bitmap_index(j + 1), :] //= 2
            marked_frame2[bitmap_index(i):bitmap_index(i + 1), bitmap_index(j):bitmap_index(j + 1), :] += np.array(color, dtype=np.uint8) // 2
        else:
            color = colors[0]
            for _i, _j in [(-1, 0), (0, -1), (+1, 0), (0, +1)]:
                _i += i
                ...
                try:
                    ...
                    pass
    marked_frame2 = np.ascontiguousarray(marked_frame2)
    cv2.imwrite(f'example-frames/bm2-{_idx:03d}.jpg', marked_frame2[5*CHUNK_SIZE:14*CHUNK_SIZE, 10*CHUNK_SIZE:23*CHUNK_SIZE])

    # highlight relevent chunks with neighbor chunks
    for i, j, chunk in chunks:
        if bitmap[i, j] == 1:
            color = colors[1]
            marked_frame3[bitmap_index(i):bitmap_index(i + 1), bitmap_index(j):bitmap_index(j + 1), :] //= 2
            marked_frame3[bitmap_index(i):bitmap_index(i + 1), bitmap_index(j):bitmap_index(j + 1), :] += np.array(color, dtype=np.uint8) // 2
    marked_frame3 = np.ascontiguousarray(marked_frame3)
    cv2.imwrite(f'example-frames/bm3-{_idx:03d}.jpg', marked_frame3[5*CHUNK_SIZE:14*CHUNK_SIZE, 10*CHUNK_SIZE:23*CHUNK_SIZE])
    
    # highlight each cell in the same group group with a same color
    ...
    cv2.imwrite(f'example-frames/bm-{_idx:03d}.jpg', marked_frame[5*CHUNK_SIZE:14*CHUNK_SIZE, 10*CHUNK_SIZE:23*CHUNK_SIZE])
    for bb in bboxes:
        x1, y1, x2, y2 = bb
    ...
    cv2.imwrite(f'example-frames/all-{_idx:03d}.jpg', marked_frame[5*CHUNK_SIZE:14*CHUNK_SIZE, 10*CHUNK_SIZE:23*CHUNK_SIZE])
    ...
    cv2.imwrite(f'example-frames/bbox-{_idx:03d}.jpg', frame[5*CHUNK_SIZE:14*CHUNK_SIZE, 10*CHUNK_SIZE:23*CHUNK_SIZE])
