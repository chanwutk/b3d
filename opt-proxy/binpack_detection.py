...
def split_image(image: npt.NDArray):
...
def decode(filename: str, inQueue: mp.Queue, bitmask: torch.Tensor, device: str):
    ...
    with torch.no_grad():
        while idx < LIMIT:
            ...
            inQueue.put((frame.detach().cpu().numpy(), idx))
            idx += 1
    inQueue.put(None)
...
def group_chunks(bitmap: npt.NDArray):
    ...
    return groups[1:h+1, 1:w+1], sorted(bins, key=lambda x: x[1].sum(), reverse=True)


def pack(bins: list[tuple[int, npt.NDArray, tuple[int, int]]], h: int, w: int, fid: int, init_bitmap: npt.NDArray | None = None):
    ...
    if init_bitmap is None:
        ...
        positions.append((fid, 0, 0))

        bins = bins[1:]


    for group, mask, _ in bins:
...
def proxy(inQueue: mp.Queue, outQueue: mp.Queue, device: str):
    proxy_model = torch.load('model.pth')
    ...
    while True:
        ...
        splitted_frame = split_image(frame)
        logQueue.put(splitted_frame.shape)
        
        splitted_frame = torch.from_numpy(splitted_frame).to(device).reshape(-1, CHUNK_SIZE, CHUNK_SIZE, 3)
        logQueue.put(splitted_frame.shape)

        splitted_frame = splitted_frame.permute(0, 3, 1, 2).to(torch.float32)

        logQueue.put(splitted_frame.shape)

        results = proxy_model(splitted_frame)
        logQueue.put(results.shape)

        results = results.squeeze()
        logQueue.put(results.shape)
        results = results.reshape(frame.shape[0] // CHUNK_SIZE, frame.shape[1] // CHUNK_SIZE)
        logQueue.put(results.shape)
        ...
        groups, sorted_groups = group_chunks(results)

        packed = pack(sorted_groups, results.shape[0], results.shape[1], results)

        # todo: binpack chunks from groups
        # greedily
        ...
        # outQueue.put((frame, bitmap, idx))


def detect(inQueue: mp.Queue, outQueue: mp.Queue, device: str):
...
def main(args: argparse.Namespace):
    ...
    imageQueue = mp.Queue()
    pDecode = mp.Process(target=decode, args=(videofile, imageQueue, bitmask, 'cuda:0'))
    pDecode.start()

    proxyQueue = mp.Queue()
    pProxy = mp.Process(target=proxy, args=(imageQueue, proxyQueue, 'cuda:1'))
    pProxy.start()
