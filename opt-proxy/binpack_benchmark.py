...
for idx in range(LIMIT):
    ...
    chunk_list.append(grouped_chunks)
    try:
        combined_chunk_list = list(itertools.chain(*chunk_list))
        grouped_chunks = pack(sorted(combined_chunk_list, key=lambda x: x[1].sum(), reverse=True), bitmap.shape[0], bitmap.shape[1])
        pass
    except ValueError as e:
        pack_values.append(len(chunk_list) - 1)
