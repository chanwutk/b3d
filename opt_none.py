...
def main(args):
    ...
    tracker = StrongSORT(width, height)
    fe = FeatureExtractor(WEIGHTS / 'osnet_x0_25_msmt17.pt', 'cuda:1', False)
    masker = None
    start = time.time()
    ...
    with torch.no_grad():
        while cap.isOpened() and frame_index < 3000:
            ...
            tracked_objects = tracker.update(torch.from_numpy(_detections), [f'{frame_index}-{d}' for d in range(len(detections))], features)
